...
短いセッション名を尋ねられた後、設計のディシジョンツリーを枝ごとに一問一答で掘り下げます。質問は Claude Code の **`AskUserQuestion`（選択肢 UI）** で提示され、**推奨案が先頭の選択肢（「（推奨）」付き）**として並ぶので、基本は**選ぶだけ**で答えられます（自由記述したいときは自動で出る「Other」を使えます）。長いセッションでは `grill-me-sessions/<プラン名>.grill.md` に決定・保留・未解決を記録します。
