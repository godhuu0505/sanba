...
CAT_LABEL = {
    "A1": "アクセス制御/IDOR/BOLA", "A2": "認証・セッション", "A3": "インジェクション/XSS",
    "A4": "プロンプトインジェクション/過剰エージェンシー", "A5": "SSRF", "A6": "暗号・秘密の扱い",
    "A7": "機微情報の露出/PII", "A8": "入力検証・逆シリアライズ・ファイル処理", "A9": "設定ミス",
    "A10": "コメント内機微情報", "B": "バグ/境界/並行性/リソースリーク", "C": "過度な複雑性",
    "D": "デッドコード/不要処理", "E": "可用性・耐障害性", "F": "サプライチェーン/CI",
}
...
def matrix(conf, out):
    ...
    for c in cats_present(conf):
        ...
        out.append(f"| {c} | {CAT_LABEL.get(c, c)} | {p1} | {p2} | {len(cf)} |")
    tp1 = sum(1 for f in conf if sev(f) == "P1")
    tp2 = sum(1 for f in conf if sev(f) == "P2")
    out.append(f"| **計** | | **{tp1}** | **{tp2}** | **{len(conf)}** |")
...
def cmd_reports(audit, units, targets, dest, head):
    ...
    L.append("- 方法: マルチエージェント並列（発見 → 敵対的検証）。事実記述のみ、対応方針は含めない。")
    t = audit["totals"]
    L.append(f"- 確定 {t['confirmed']} 件（P1 {t['p1']} / P2 {t['p2']}） / 要確認 {t['uncertain']} / 棄却 {t['refuted']}\n---\n")
    for label, sv in [("P1（重大度高）", "P1"), ("P2（重大度中〜低）", "P2")]:
        ...
        for c in cats_present(grp):
            ...
            for f in cf:
                L.append(f"#### {f['_id']} `{f['file']}:{f.get('line')}` — {esc(f.get('title'))}")
    ...
    for i, f in enumerate(audit["uncertain"], 1):
        L.append(f"#### U-{i:02d} `{f['file']}:{f.get('line')}` — {esc(f.get('title'))}")
        L.append(f"- 観点/フレームワーク: {f.get('category')} / {f.get('framework') or '-'}")
        L.append(f"- 事実: {esc(f.get('fact'))}")
    ...
    L.append(f"- 対象 HEAD: `{head}`")
    L.append(f"- 監査対象ファイル総数: **{len(targets)}**（テスト・ロック・バイナリ・md を除く全ソース）")
    L.append("- ✓=担当エージェントが全行 Read。指摘数はそのファイルに紐づく確定 finding 件数。\n")
    checked = 0
    file_to_unit = {}
    for u in units["units"]:
        for f in u["files"]:
            file_to_unit[f] = u["name"]
    for u in units["units"]:
        ...
        for rf in u["files"]:
            ...
            checked += 1
            L.append(f"| `{rf}` | ✓ | {len(fids)} | {', '.join(fids) if fids else '-'} |")
        L.append("")
    L.append("---\n## 集計\n")
    L.append(f"- 対象総数: **{len(targets)}**")
    L.append(f"- 確認済み: **{checked}**")
    L.append(f"- 未確認: **{len(targets) - checked}**")
    L.append(f"- 確定 finding が紐づくファイル数: **{len(byfile)}** / 確定 finding 総数: **{len(conf)}**\n")
    open(os.path.join(dest, "coverage-log.md"), "w").write("\n".join(L))
    print(f"wrote {dest}/findings.md, {dest}/coverage-log.md (targets {len(targets)}, checked {checked})")
...
def cmd_summary(audit, units, targets, dest, head):
    ...
    L.append("- 方法: マルチエージェント並列（発見単位が担当ファイルを全行読了 → 生指摘 → 各指摘を独立エージェントが該当コード再読で敵対的検証）。")
    L.append("- 制約: 判断根拠は現在のソースコードのみ。docs/ADR・ソース中コメントの説明は挙動判断に不使用。対応方針は含めない。\n")
    L.append("## 結果総数\n")
    L.append("| 区分 | 件数 |\n|---|---|")
    ...
    for u, n in Counter(f.get("unit") for f in conf).most_common():
        L.append(f"| {u} | {n} |")
    L.append("")
    L.append("## P1 一覧（詳細は findings.md）\n")
    L.append("| ID | ファイル:行 | 観点 | 指摘 |\n|---|---|---|---|")
    for f in [x for x in conf if sev(x) == "P1"]:
        L.append(f"| {f['_id']} | `{f['file']}:{f.get('line')}` | {f.get('category')} | {bar(f.get('title'))} |")
    L.append("")
    os.makedirs(dest, exist_ok=True)
...
def cmd_issue_body(audit, head, out):
    ...
    t = audit["totals"]
    L = []
    L.append(f"セキュリティ監査（HEAD `{head}`）で確定した指摘の追跡 issue。**事実の記述のみ**で対応方針は含めない。詳細は `security-audit/findings.md`。\n")
    L.append("## 総数\n")
    L.append(f"- 確定 **{t['confirmed']}** 件（P1 {t['p1']} / P2 {t['p2']}） / 要確認 {t['uncertain']} / 棄却 {t['refuted']}")
    ...
    L.append("## P1 追跡\n")
    for f in [x for x in conf if sev(x) == "P1"]:
        L.append(f"- [ ] **{f['_id']}** `{f['file']}:{f.get('line')}` ({f.get('category')}/{f.get('framework') or '-'}) — {esc(f.get('title'))}")
    L.append("")
    L.append("## P2 追跡\n")
    for f in [x for x in conf if sev(x) == "P2"]:
        L.append(f"- [ ] {f['_id']} `{f['file']}:{f.get('line')}` ({f.get('category')}) — {esc(f.get('title'))}")
    L.append("")
    body = "\n".join(L)
    if out:
...
def cmd_adr(audit, number, adr_dir, head):
    ...
    L = []
    L.append(f"# ADR-{nnnn}: 全コード セキュリティ監査プロセス（マルチエージェント並列 + 敵対的検証）\n")
    L.append("- ステータス: Proposed")
    L.append("- 日付: TODO(YYYY-MM-DD を起票時に記入)\n")
    L.append("## コンテキスト\n")
    L.append("全ソースコードに対する定期的なセキュリティ監査を、再現可能で網羅性を機械的に追える形で回したい。")
    L.append("単一エージェントでの逐次レビューは大規模コードベースで漏れ・見落としが生じやすく、指摘の真偽（誤検知）も判別しづらい。\n")
    L.append("## 決定\n")
    L.append("`/security-audit` スキルの手順を標準の監査プロセスとして採用する:")
    L.append("1. `partition.py` で監査対象（テスト/ロック/バイナリ/md を除く全ソース）を列挙し、モジュール/ディレクトリ境界で単位分割する。")
    L.append("2. Workflow オーケストレーションで、発見エージェントが担当ファイルを全行読了し、観点 A〜F（OWASP Top10 / CWE Top25 / API Security Top10 / LLM・Agentic Top10 / ASVS / GitHub Actions hardening）で指摘する。")
    L.append("3. 各指摘を独立した検証エージェントが該当コードを再読して敵対的に検証し、誤検知を棄却する。")
    L.append("4. 確定指摘・全ファイル確認証跡・サマリーを `security-audit/` に出力し、GitHub issue（事実の追跡）と本 ADR（プロセス記録）に反映する。")
    L.append("- 監査は事実記述のみとし、対応方針は含めない（判断は人間が行う。CLAUDE.md 原則1）。")
    L.append("- 判断根拠は現在のソースコードのみ。ソース中コメントは挙動判断に使わず、機微情報漏洩の観点でのみ走査する。\n")
    L.append(f"直近の実行結果: 確定 {t['confirmed']} 件（P1 {t['p1']} / P2 {t['p2']}）、要確認 {t['uncertain']}、棄却 {t['refuted']}。詳細は issue と `security-audit/` を参照。\n")
    L.append("## 検討したが採用しなかった選択肢\n")
    L.append("- 単一エージェントの逐次レビュー: 網羅性・スケールと誤検知判別に難があり却下。")
    L.append("- SAST ツールのみ: パターン検出は有効だがビジネスロジック・LLM 特有のプロンプトインジェクション・過剰エージェンシーの判定に弱く、補完に留める。")
    L.append("- 監査時に対応方針まで自動生成: 設計判断は人間が行う原則に反するため、事実提示までをスコープにする。\n")
    L.append("## 影響\n")
    L.append("- 観測性: 監査は静的解析であり実行時トレースは追加しない。実行時挙動（デプロイ env 値・IAM・Firestore ルール）は監査対象外で、条件付き事実として記述する。")
    L.append("- テスト/CI: 監査は CI ゲートではなくオンデマンド/定期実行。フル実行は大きなトークン消費を伴うため、単位分割と縮小スモークを併用する。")
    L.append("- フォローアップ: 確定指摘は issue で追跡し、対応は別 PR で行う。監査結果スナップショットは別途 ADR 化しない（本 ADR はプロセスの記録）。\n")
