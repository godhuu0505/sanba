...
async def add_context_file(
    ...
    DELETE での破棄に対応）。画像/動画は Cloud Storage に保存し、安定 `asset_id` を返す
    （issue #103 / ADR-0004）。画像は Gemini で観察を抽出して grounding に流し、agent が
    問いの根拠にできるようにする。動画解析は未実装のため保存のみ（`analysis_pending=true`、
    web では「準備中」）。非対応形式は 415 で弾く。
