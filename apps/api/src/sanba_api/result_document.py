"""要件結果ドキュメントの整形（audience 別出力フォーマット）。

アプリ管理画面で登録された（または既定の）出力フォーマット（Markdown テンプレート）へ、
確定要件・確認項目などを機械的に埋め込む。LLM 追加呼び出しはしない（repo 要約シードや
`github_export.requirements_to_issue_body` と同じ方針）。純粋な整形関数のみで、
ネットワーク・Firestore に依存しない（単体テスト対象）。

テンプレートのプレースホルダ仕様は `sanba_shared.output_formats` の docstring を正とする。
"""
...
# 企画者にもそのまま見せられるよう MoSCoW に日本語注記を併記する（github_export の
# Issue 見出しは英語のみだが、こちらは閲覧ドキュメントなので読み手に寄せる）。
_PRIORITY_LABELS = {
    "must": "Must（必須）",
    "should": "Should（重要）",
    "could": "Could（できれば）",
