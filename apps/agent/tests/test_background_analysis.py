...
def _end_user_agent(transport: RecordingTransport | None = None) -> SANBAAgent:
...
@pytest.mark.asyncio
async def test_end_user_session_suppresses_nfr_gap_detections() -> None:
    """end_user では企業向け NFR gap を出さず、終了提案を永久ブロックしない（#434 / ADR-0055）。"""
    transport = RecordingTransport()
    agent = _end_user_agent(transport)
    result = AnalysisResult(
        summary="s",
        open_topics=["性能・レイテンシの要件", "可用性・SLO", "セキュリティ・プライバシー"],
        next_question="q?",
        suggested_answer="a",
    )
...
@pytest.mark.asyncio
async def test_end_user_session_still_surfaces_ambiguous_detections() -> None:
    """曖昧語検知はモード非依存なので end_user でも従来どおり出す。"""
    transport = RecordingTransport()
    agent = _end_user_agent(transport)
    result = AnalysisResult(
        summary="s",
        open_topics=["性能・レイテンシの要件"],
        ambiguous_topics=["いい感じにしてほしい"],
        next_question="q?",
        suggested_answer="a",
    )
    await agent._publish_analysis_detections(result)
    assert agent._open_detection_count() == 1
    assert any(t["event"]["type"] == "detection.ambiguous" for t in transport.sent)
    assert not any(t["event"]["type"] == "detection.gap" for t in transport.sent)


@pytest.mark.asyncio
async def test_developer_session_still_surfaces_nfr_gap_detections() -> None:
    """developer では従来どおり NFR gap を出す（end_user ゲートの回帰防止）。"""
    transport = RecordingTransport()
    agent = _agent(transport)
    result = AnalysisResult(
        summary="s",
    ...
    await agent._publish_analysis_detections(result)
    assert agent._open_detection_count() == 1
    assert any(t["event"]["type"] == "detection.gap" for t in transport.sent)
