...
- 役割の位置づけは [`docs/architecture.md`](../../docs/architecture.md) を参照。
...
src/sanba_agent/
  ...
  prompts/         プロンプト（バージョン管理 + Langfuse Prompts）
  ...
  observability.py OpenTelemetry / Langfuse 計装
...
| `LANGFUSE_*` | LLM トレース/評価 | 送信スキップ |
| `MASK_PII_BEFORE_INDEX` | PII マスク | 既定 `true` |

`.env.example` が正。詳細は [`docs/local-dev.md`](../../docs/local-dev.md) / [`docs/security.md`](../../docs/security.md)。
