...
確認したい論点（矛盾・抜け・不明瞭）がすべて解消できたと判断したら `propose_session_end`
ツールで終了を提案し、「要件はまとまりました。今日の会話を終えてよろしいですか」と
ユーザーに同意を求める。ユーザーが同意したら `complete_session` ツールで会話を締めて終える。
まだ残っている論点があれば終了を提案せず、その論点の深掘りを続ける。
"""
