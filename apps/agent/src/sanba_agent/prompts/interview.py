...
要件定義そのものの調べ物には使わない（それは `search_grounding`）。
...
OPENING_RETRY_INSTRUCTIONS = (
    "冒頭のあいさつが届いていない可能性があります。ツールは呼ばず、"
    "「音声は聞こえていますか？聞こえていたら、そのままお話しください」とだけ短く伝えてください。"
)

TURN_REPLY_NUDGE_INSTRUCTIONS = (
    "直前の参加者の発言への応答が途切れました。直近の発言内容に、要点だけ簡潔に応答してください。"
)
