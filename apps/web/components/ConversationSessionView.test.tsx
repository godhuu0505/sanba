...
describe("ConversationSessionView（会話シェル結線）", () => {
  ...
  it("未解消0件なら終了するで直接確定し、結果へ進む", async () => {
  ...
  it("未解消0件で終了すると onFinalize を呼んでから結果へ進む（#186）", async () => {
  ...
  it("未解消0件でも確定が失敗したら forced 確定を試みてから結果画面へフォールバックする（#186 / Codex P2）", async () => {
    ...
    const onNavigateResults = vi.fn();
    renderView({ state: baseState({ inquiryNodes: [] }), onFinalize, onNavigateResults });
    fireEvent.click(screen.getByRole("button", { name: "会話を終了" }));
    fireEvent.click(screen.getByRole("button", { name: "終了する" }));
    await waitFor(() => expect(onNavigateResults).toHaveBeenCalledTimes(1));
...
describe("ConversationSessionView（セッション終了後の閲覧モード）", () => {
  ...
  it("未解消のまま終えた後（router 無しフォールバック）はツリーを閲覧のみで出し、問いピンと『不要』を出さない", async () => {
    renderView();
    fireEvent.click(screen.getByRole("button", { name: "会話を終了" }));
    fireEvent.click(screen.getByRole("button", { name: "終了する" }));
...
describe("ConversationSessionView（読取専用ゲスト / ADR-0032 決定4）", () => {
  ...
  it("read-only は provisional へフォールバックしつつルームは切断する（③ ゲスト経路 / P0 リーク防止）", async () => {
    const onNavigateResults = vi.fn();
    const onEndSession = vi.fn();
    renderView({ readOnly: true, onNavigateResults, onEndSession });
    fireEvent.click(screen.getByRole("button", { name: "会話を終了" }));
    fireEvent.click(screen.getByRole("button", { name: "終了する" }));
...
describe("ConversationSessionView（end_user モード語彙 / FR-2.4）", () => {
  ...
  it("利用者向けモードでも終了→結果へ進む（Must 内訳を出さない）", async () => {
