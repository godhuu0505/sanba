...
/**
 * SANBA 体験（dark + gold）の地となるサーフェス。
 *
 * 各画面（ホーム/会話/管理 等）のルートに敷き、配下に放射グラデの下地・明朝書体・
 * 生成りの既定テキスト色という「文脈」を一括で与える。これにより個々の子要素は
 * 色やフォントを再指定せずに済み、light テーマの admin/login とは隔離される。
 *
 * - `bordered` を付けると Figma 正本の金枠付き端末フレームになる（ショーケース用）。
