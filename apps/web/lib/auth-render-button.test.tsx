...
// ADR-0019 の承認バリアント引数（theme:"filled_black" / text:"continue_with" /
// locale:"ja"、size/shape は維持）で呼ばれることを担保する。CLIENT_ID はモジュール
// 評価時に env を読むため、stubEnv → resetModules → 動的 import で real モードに入る。
...
describe("useGoogleAuth renderButton 引数（real モード / ADR-0019）", () => {
  it("theme:filled_black / text:continue_with / locale:ja で純正ボタンを描画する（size/shape 維持）", async () => {
    ...
    // ブランド規約: 旧 outline / signin_with へ戻していない（地色・文言を承認バリアントに固定）。
    expect(options).not.toMatchObject({ theme: "outline" });
    expect(options).not.toMatchObject({ text: "signin_with" });
  });
