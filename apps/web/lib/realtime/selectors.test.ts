...
describe("selectActiveQuestion", () => {
	...
	it("選択肢なし（自由記述）は問いピン対象にしない（null）", () => {
		expect(
			selectActiveQuestion(state({ id: "q1", prompt: "自由に", options: [] })),
		).toBeNull();
	});
...
describe("mergeMaterials", () => {
	...
	it("同一 asset_id は status/pct を realtime（ライブ）優先で統合する", () => {
		const realtime = [
			item({ id: "a1", name: "a1", pct: 100, status: "done", extracted: 2 }),
		];
		const hydrated = [
			item({ id: "a1", name: "mock.png", pct: 0, status: "analyzing" }),
		];
	...
	it("local（投入直後）の実ファイル名も realtime 行に引き継ぐ", () => {
		const realtime = [
			item({ id: "a2", name: "a2", pct: 50, status: "analyzing" }),
		];
		const local = [
			item({ id: "a2", name: "diagram.png", pct: 100, status: "analyzing" }),
		];
	...
	it("cancelledIds の素材は除外する（遅延 realtime が来ても復活しない）", () => {
		...
		const local = [
			item({ id: "a1", name: "mock.png", pct: 0, status: "uploading" }),
		];
	...
	it("cancelled 行は同 id の realtime 後勝ちでも復活しない（cancelledIds 未指定・Codex P2）", () => {
		...
		const realtime = [
			item({ id: "a1", name: "a1", pct: 60, status: "analyzing" }),
		];
...
describe("selectMaterials", () => {
	...
	it("解析途中（pct<100）は analyzing・抽出数なし", () => {
		expect(
			selectMaterials({
				analysis: [analysis({ asset_id: "a1", pct: 40, stage: "領域検出" })],
			}),
	...
	it("完了（pct>=100）は done・抽出要件数を件数で返す", () => {
		expect(
			selectMaterials({
				analysis: [
					analysis({ asset_id: "a2", pct: 100, extracted: ["x", "y", "z"] }),
				],
			}),
		).toEqual([
			{ id: "a2", name: "a2", pct: 100, status: "done", extracted: 3 },
		]);
	});
	...
	it("完了でも抽出が空なら extracted は付けない", () => {
		expect(
			selectMaterials({
				analysis: [analysis({ asset_id: "a3", extracted: [] })],
			}),
		).toEqual([{ id: "a3", name: "a3", pct: 100, status: "done" }]);
	});
	...
	it("stage='failed' は pct=100 でも failed（done と誤判定しない）", () => {
		expect(
			selectMaterials({
				analysis: [analysis({ asset_id: "a4", pct: 100, stage: "failed" })],
			}),
	...
	it("stage='received' は解析中（受領直後・pct=10）", () => {
		expect(
			selectMaterials({
				analysis: [analysis({ asset_id: "a5", pct: 10, stage: "received" })],
			}),
		...
		const detail = selectMaterialDetail(
			{
				analysis: [
					analysis({
						...
						conflicts: [
							{ summary: "検索バーが無いが『検索したい』と発言", refs: ["u1"] },
						],
					}),
				],
			},
...
describe("selectMiniStatus", () => {
	...
	it("空の状態は全て 0 / false", () => {
		expect(
			selectMiniStatus({ requirements: [], detections: [], analysis: [] }),
		).toEqual({
			requirements: 0,
			unresolved: 0,
			materials: 0,
