...
from .analytics import TokenUsage, usage_from_genai
...
def _generate_config(labels: dict[str, str] | None) -> Any | None:
    if not labels:
        return None
...
def _report_usage(resp: Any, on_usage: Callable[[TokenUsage], None] | None) -> None:
    if on_usage is None:
        return
    try:
        on_usage(usage_from_genai(getattr(resp, "usage_metadata", None)))
    except Exception as exc:  # noqa: BLE001
        log.warning("vision_usage_hook_failed", error=str(exc))
...
def analyze_image(
    ...
    *,
    ...
    `on_usage` はトークン usage の記録（ADR-0061 の `ai_usage` 排出）、`billing_labels` は
    Vertex 経路の請求ラベル付与に使う。どちらも失敗しても解析本体は止めない。
    """
    ...
    try:  # pragma: no cover
        ...
        resp = client.models.generate_content(
            ...
            config=_generate_config(billing_labels),
        )
        _report_usage(resp, on_usage)
