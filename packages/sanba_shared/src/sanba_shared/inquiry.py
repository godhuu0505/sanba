...
class InquiryTree:
    ...
    def open_nodes(self, kinds: tuple[InquiryKind, ...] | None = None) -> list[InquiryNode]:
        """open なノードを created_seq 昇順で返す（kinds 指定でその種別だけ）。"""
        return [
            n
            for n in self.nodes()
            if n.status is InquiryStatus.OPEN and (kinds is None or n.kind in kinds)
        ]

    def resolve_best_match(
        self,
        kinds: tuple[InquiryKind, ...],
        text: str,
        seq: int,
        *,
        ...
        ranked = sorted(
            (
                (SequenceMatcher(None, norm, normalize_text(n.text)).ratio(), n)
                for n in self.open_nodes(kinds)
            ),
