...
- **(b) 操作者の GitHub 権限が product の紐づけ repo を含む**（本人 installation で対象 repo を
  読めるか。単一 `GET /repos/{repo}` の可否〔200/403・404〕で判定し、全 repo 列挙は避ける）。
...
（`/results/[id]`）で要件を確認して Issue 登録」とする。結果レビュー画面は**承認機能を持たず**、
要件結果の確認と起票のみを担う。起票導線は次の 2 系統:

- **権限を持つ開発者**（連携済み ∧ 対象 repo 権限あり）: 画面下部に**アクティブな「Issue を作成」
  ボタン**を出し、ログイン資格（idToken）で認可する `POST /api/sessions/mine/{id}/export` を叩く
  （セッショントークンが残らない後追い閲覧のため、`/mine/` 系と同じ owner_sub 一致で認可）。
- **権限を持たない利用者・メンバー**: ボタンは**ディセーブル**にし理由を表示。代わりに要件結果を
  **Markdown でコピー**でき（既存の結果ドキュメント出力）、GitHub の Issue 作成画面に貼り付けて
  手動起票できる導線を案内する。

Issue の**作成者表示は `SANBA[bot]`**（installation token のため）とし、誰が起票したかは Issue
本文に「SANBA セッション {id} / export by {github_login}」の形で記録する（案 B のまま。実在の
...
- `apps/api`:
  ...
  - `routers/sessions.py`（`/export`）: 共有 PAT を操作者本人の installation token に置換、
    Issues:write で起票。起票の実処理を `_perform_export` に集約し、可否判定 `export_eligibility`
    （`deps.py`）を追加。共有 PAT 書き経路の退役。
  - 起票可否 API（ボタン活性判定用）: セッショントークン経路の
    `GET /api/sessions/{id}/export/eligibility`（ライブ結果画面）と、ログイン資格・owner_sub 認可の
    `GET|POST /api/sessions/mine/{id}/export(/eligibility)`（後追いの結果レビュー画面）。
- `apps/web`:
  ...
  - ライブ結果画面（`ResultView` / `ConversationSessionView`）: eligibility を取得し Issue ボタンの
    活性/非活性と理由表示（決定 4）。理由文言は `lib/issueExport.ts` に集約。
  - 結果レビュー画面（`/results/[id]`）: 権限があれば「Issue を作成」を活性化、無ければディセーブル＋
    既存の Markdown コピーで手動起票を案内（決定 5）。
