...
**更新（トークンリフレッシュ方針 / ADR-0046）**: 本節は「失効したら再サインインを促す」事後
対応だったが、開いたままの長い会話の途中で ID トークン（約1h）が失効すると LiveKit 再 join や
create/join が 401 で刺さる。これを防ぐため **exp を先読みして失効前に GIS で静かに再取得する
能動リフレッシュ** を追加する（ADR-0046 決定1）。あわせて ID トークン注入対策として、サーバ
発行の HMAC nonce を GIS に渡し、create/join で `nonce` claim を照合する（ADR-0046 決定2。
段階リリースフラグ `REQUIRE_LOGIN_NONCE`）。**いずれも ID トークンを localStorage に置かない・
メモリのみ、という本節の方針は不変**（更新するのはメモリ内のトークンで、永続化はしない）。
