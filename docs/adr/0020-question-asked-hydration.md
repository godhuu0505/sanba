...
- **棄却した live クリアでも `maxSeq` は進める**: id 不一致・`cleared_seq <= lastQuestionSeq`・`current=null`（ask を
  取り逃した）でピンを畳まない場合でも、live `question.cleared` を受信した事実として **`maxSeq = Math.max(maxSeq,
  event.seq)`** を進める。既存 store は `reduce()` が false だと `maxSeq` を更新しない（`store.ts:206-213`）ため、
  これを怠ると受信済みの `event.seq` を欠番扱いし、次の `event.seq+1` で不要な gap/再ハイドレーションを発火する
  （§5-2 の「適用有無に関わらず maxSeq 前進」と同じ原則をクリアにも適用）。
...
- **規律**: question hydration は **`lastQuestionSeq`（question 専用ガード）は常に**進めてよいが、**global `maxSeq` は
  主スナップショット（`/requirements`＝session 境界の出所）が成功したときだけ**進める。主 GET が失敗した hydration
  パスでは global `maxSeq` を question 由来 seq で進めず、gap/retry を残す。
...
1. **共有/agent**（`packages/sanba_shared/.../repository.py`、`apps/agent/.../events.py`・`main.py`）:
   ...
   - **§5-5 / 5-9 / cleared_seq 単一予約**: クリア時に **`question.cleared`（live イベント / `question_id` + `cleared_seq`）を
     ...
     **envelope seq そのもの**（二重採番しない）で、tombstone・live・GET の seq に同一値を使う。順序は **予約 →
     tombstone commit → publish**、commit 失敗時は publish しない（§5-1 と対称）。
...
3. **Web**（`apps/web/lib/realtime/store.ts`・`useRealtimeSession.ts`）: `hydrateQuestion(question, seq)`
   ...
   **主スナップショット成功時のみ**＝§5-11）。**クリア適用は §5-10 のガード**で統一: `null` 適用と `question.cleared` 受信は
   `cleared_seq > lastQuestionSeq`（live は加えて `question_id === current?.id`）のときだけ畳み、適用時に
   **`lastQuestionSeq` と `maxSeq` の両方を `cleared_seq` まで前進**。**畳まない live クリアでも `maxSeq` は前進**（§5-10）。
   接続時 GET の結線（他 GET と同じ順序）。
4. **テスト**: agent の保存/クリア単体（§5-3 id 不一致でクリアしない・§5-1 保存→送信順・§5-1 保存失敗で seq 欠番
   ...
   クリア済みを復活させない・古い `question.cleared` で新しい問いを畳まない・畳まない clear でも maxSeq 前進**・**§5-11: 主
   GET 失敗時は question seq で maxSeq を進めず gap/retry を残す**）。
