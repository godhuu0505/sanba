...
- 適用の有無に関わらず **`maxSeq = Math.max(maxSeq, seq)`** を進める（誤 gap 防止）。
...
null を適用したときも **`maxSeq = Math.max(maxSeq, cleared_seq)`** を進める（§5-2 と対称。これを怠ると
クリア後の次イベント `N+2` を受けるたびに未受信の `N+1` と誤判定して不要な gap/再ハイドレーションを起こす）。
...
`cleared_seq`）を追加**し（WAVE2・契約 §2 への追記）、web は受信で当該ピンを畳む。`question.cleared` は
seq 境界を進めない点で `question.asked` と対称に扱う（§3）。※既存 3 API・既存イベントの仕様は変えない（新規追加のみ）。

**5-6. 自由記述（options なし）回答でも current question をクリアする。**
クリア契機を `user.answered` 受信に限定すると漏れる: `question.asked` は options なしの自由記述を許すが、
`ChoicePin` は options が空だとタップ UI を描かず、ボトムバー `user.text` や音声回答には `question_id` が無いため
`record_answer` に到達しない。結果、自由記述で答えた質問が Firestore に残り、リロード/途中参加で回答済みの問いが
復活する。対応方針: agent は**現在 current question が未回答の間に届いた `user.text`/音声発話を、その current
question への回答とみなして対応付け、クリアする**（最新1問モデルでは「次の発話＝直近の問いへの応答」が自然）。
options あり質問は `question_id` 一致で（§5-3）、options なし質問は「未回答 current への次回答」でクリアする。
...
**5-8. `current_question` は保持期限（TTL / 削除）を必須にする（PII を放置しない）。**
...
あるいはフィールド案を採るなら**明示的な削除/マスク/保持期限を必須条件**とする。`approved` のような保全対象では
ないため、未回答のまま離脱したら他の一過性データと同じく TTL で消える運用にする（issue #10 のデータ最小化と整合）。
...
1. **共有/agent**（`packages/sanba_shared/.../repository.py`、`apps/agent/.../events.py`・`main.py`）:
   ...
   - **§5-3 / 5-7**: クリアは現在質問 id == `question_id` のときのみ。id 照合 + 削除 + `cleared_seq` 記録は
     **Firestore transaction / CAS** で原子的に行う（`ask_question` の上書きとの競合回避）。
   - **§5-5**: クリア時に **`question.cleared`（live イベント / `question_id` + `cleared_seq`）を publish**して
     全参加者へ伝播（契約 §2 に新規追加）。seq 境界は進めない。
   - **§5-6**: options なし質問は、未回答 current の間に届いた `user.text`/音声発話を当該 current への回答と
     みなしてクリアする経路を実装。
...
3. **Web**（`apps/web/lib/realtime/store.ts`・`useRealtimeSession.ts`）: `hydrateQuestion(question, seq)`
   （**`seq > lastQuestionSeq` のときだけ** `this.question`/`lastQuestionSeq` を更新＝§5-2、適用有無に関わらず
   `maxSeq = Math.max(maxSeq, seq)`）、`null` 適用も `seq > lastQuestionSeq` 条件 + `maxSeq` 前進（§5-4）、
   `question.cleared` live 受信でピンを畳む（§5-5）、接続時 GET の結線（他 GET と同じ順序）。
4. **テスト**: agent の保存/クリア単体（§5-3 id 不一致でクリアしない・§5-1 保存→送信順・§5-1 保存失敗で seq 欠番
   なし・§5-7 並行回答とトランザクション・§5-6 自由記述回答でクリア）、API GET 単体（Bearer 必須 / 未提示・回答済みで
   null + `seq`）、web store 単体（古いライブ/古い GET は破棄・新しい問いで置換・**復元直後の `N+1`/`N+2` で誤 gap を
   出さない**§5-2/5-4・遅延 null が新しい問いを消さない・`question.cleared` で畳む§5-5）。
