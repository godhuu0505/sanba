...
- **委譲の監査**: ファサードは委譲レコード（question / result / caller / status / 所要）を sanba-ops
  Firestore へ冪等 upsert する（RUNNING → DONE/ERROR。doc id は task_id、書き込み失敗は fail-soft）。
  永続化前に question / result を PII マスク（facade 内の軽量 `pii.py`。最小ベース維持のため
  sanba_shared は取り込まない）、`expire_at`（既定 30 日）で Firestore TTL を効かせる。委譲元の
  追跡のため SANBA セッション ID を `SendMessageRequest.metadata.caller` で伝搬し `caller` に刻む
  （owner email 等の PII はマスク対象）。ロールは専用最小 SA（`holmes-invoker`・prod）を新設し
  ops ファサードの `run.invoker` を付与、ファサード SA には `roles/datastore.user` を付ける。
- **切断時の挙動**: セッション切断のドレン猶予は短く、進行中の `run_investigation`（最大 300 秒）は
  基本キャンセルされる。`release_investigation` は `finally` で保証されリークはしないが、切断後に
  結果が届くことは保証しない（届いた頃には会話が無い）。off-loop の他タスクと同じ割り切り。
  agent 側 httpx タイムアウト（既定 330 秒）は facade の調査許容（300 秒）より長くし、正当な
  長時間調査が agent 側で先に切れて偽「失敗」になるのを防ぐ。
