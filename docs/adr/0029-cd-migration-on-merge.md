...
- 改訂（2026-07-04）: ADR-0026（PR #284）により **Terraform 管理の宣言的設定（Firestore TTL /
  ...
  新フロー前提に更新した。
- 改訂（2026-07-07）: **「本番 KB 空」latent バグ（§下記）は先行して解消済み**。`seed_knowledge_base()`
  を is_memory に関わらず実行し、ES 経路では `knowledge:<source>` の**決定的 _id で upsert**する
  ようにした（`index_passage(doc_id=...)`）。複数インスタンスの起動時シードでも重複しないため、
  本 ADR が計画した専用 Cloud Run Job（`scripts/seed_kb` / `google_cloud_run_v2_job.seed_kb`）を
  待たずに ES 有効化で KB が満たされる。トレードオフは agent cold-start ごとの 5 回の埋め込み
  （軽微・許容）。**マッピング進化に伴う versioned index + reindex の枠組みは本 ADR の将来課題として残す**
  （KB 空バグとは別問題）。有効化手順は `docs/how-to/deploy-gcp.md §6.8`。
...
- **隠れた本番バグ（調査で発覚 → 2026-07-07 解消済み）**: かつて `scripts/seed_kb` は `main.py` の
  コメントから参照されるが**リポジトリに存在せず**、本番(非memory)では KB が seed されず
  **本番 ES の知識ベースが空のまま**になりうる latent バグがあった。現在は `seed_knowledge_base()` を
  is_memory に関わらず実行し、ES 経路は決定的 _id で冪等 upsert するため、起動時シードだけで解消する
  （上記 2026-07-07 改訂）。専用 seed スクリプト/Job は不要になった。
