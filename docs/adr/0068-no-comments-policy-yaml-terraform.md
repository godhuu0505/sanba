# ADR-0068: コメント禁止規約の YAML / Terraform への拡張とコメント内容の docs 移設
...
3. **既存コメントの内容は削除ではなく docs へ移設する**。移設先の対応:
   ...
   - agent-standby の設計メモ（2 段反映・api/web ウォームアップ） →
     `docs/reference/ci-cd-workflows.md`（運用の現況は `docs/how-to/pre-launch-cost-controls.md` §2 が正）
