...
8. **grounding の出力制御**: end_user モードでは repo 由来（ADR-0028 の索引）の passage を
   ...
   passage は従来どおり引用できる。
   **実装（PR8 改訂）**: 音声応答（speech-to-speech）は事後フィルタできないため、
   「本文はモデルに渡すが引用は禁止」というプロンプト遵守頼みの設計はフェイルオープンであり
   採らない。出力制御は `search_grounding` の**返り値の allowlist** で機械的に行う:
   end_user モード（およびセッション文書が読めずモード未確認のとき）は利用者由来 kind
   （`utterance` / `requirement`）だけを返し、repo 由来を含む `context`・開発語彙の
   `knowledge` は本文・source ともモデルに一切渡さない（source 文字列判定に頼らないため
   表記揺れのすり抜けが起きない）。「次に聞くことの判断材料」は、遮断した repo 由来ヒットの
...
- **end_user モードで grounding を完全遮断**: 質問の的が外れて品質が落ちる。
  ...
  （決定 8 の allowlist 実装はこの却下案とは異なる: 利用者の発話・過去セッション要件の
  呼び戻しは全面的に残り、repo 由来も件数シグナルとして問いの判断材料に使える。）
