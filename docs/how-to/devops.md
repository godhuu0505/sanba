...
- **Cloud Run**: api/web は `cpu_idle` + `min=0` で scale-to-zero（リクエスト時のみ課金）。
  agent は常駐ワーカーのため `agent_min_instances` で制御する。**既定は 1（常駐）で、CI 経由デプロイ
  （`terraform.yml`）も Terraform 変数の既定をそのまま使う**。scale-to-zero に戻す場合は
  `agent_min_instances` を 0 にする（warm/sleep 切り替え運用の現状は
  [pre-launch-cost-controls.md](pre-launch-cost-controls.md) §2 を参照）。
