...
| ~~`NEXT_PUBLIC_GOOGLE_API_KEY`~~ | **GitHub Variable では登録しない**。Picker 用ブラウザ API キーは Terraform が払い出し Secret Manager に格納、web ビルドがそこから読む（§6.9 / ADR-0049）。未構成なら「Google ドライブから選ぶ」が利用不可の案内になる |
...
| 種別 | 置き場所 | 例 |
|---|---|---|
| 非機微の設定 / 公開識別子 | **GitHub → Variables（`vars.*`）** | `GCP_PROJECT_ID` / `GCP_REGION` / `TF_STATE_BUCKET` / `PROD_DOMAIN` / `LIVEKIT_URL` / `NEXT_PUBLIC_*` / `OTEL_EXPORTER_OTLP_ENDPOINT` / `BILLING_ACCOUNT` / **`WIF_PROVIDER` / `DEPLOY_SA` / `TF_DEPLOY_SA`**（SA email・WIF パスは公開識別子） / `ACTIVE_APP_SECRET_IDS`（JSON 配列） |
...
## 6.9. Google ドライブ取り込みを有効化する（ADR-0049）

参考資料を Google ドライブから選ぶ導線（`MaterialSourceSheet`「Google ドライブから選ぶ」）は
**drive.file スコープ + Google Picker** の最小権限構成。Picker は「ユーザーの drive.file 同意
（OAuth トークン）」に加えて呼び出し元プロジェクトを示す **ブラウザ API キー** を必須とする。
このキーは JS バンドルへ焼かれ最終的に公開されるため、防御は秘匿ではなく **HTTP リファラ制限 +
API ターゲット制限** にある。値は GitHub Variables に置かず **Secret Manager を唯一の置き場**に
する（`session-signing-secret` と同じく、terraform が生成する値なので version まで管理する）。

> 配線（Drive/Picker/apikeys API 有効化・`google_apikeys_key`・Secret・web ビルドの Secret Manager
> 取得）は terraform / `deploy.yml` 側で完成済み。以下は**同意画面のスコープ追加と apply だけ**。

**手順**

1. **`terraform apply`**（Actions → Terraform → `apply`、または §6 のローカル apply）。これで
   `drive.googleapis.com` / `picker.googleapis.com` / `apikeys.googleapis.com` が有効化され、
   リファラ制限（web オリジンのみ）+ API 制限（Drive/Picker のみ）付きの `sanba-picker-browser-key`
   と Secret `sanba-next-public-google-api-key`（値まで terraform が格納）が作られる。web ビルド SA
   （`vars.DEPLOY_SA`）にこの Secret の read が付与される。

2. **OAuth 同意画面に `drive.file` を追加**（手動・一度きり）。APIs & Services → OAuth consent
   screen → スコープに `.../auth/drive.file` を追加する。**`drive.file` は非センシティブ**スコープで、
   非センシティブのみを使うアプリは **OAuth アプリ審査（verification）が不要**・未確認アプリ警告も
   出ない（`drive.readonly` のような restricted 審査/CASA を回避する ADR-0049 決定3の狙い）。
   本アプリの User type は **External / 一般公開**（ログインで既に `openid/email/profile` を使用）。

3. **web を再ビルド/デプロイ**（`deploy.yml`）。web ビルドは WIF で
   `gcloud secrets versions access latest --secret=sanba-next-public-google-api-key` を読み、
   `NEXT_PUBLIC_GOOGLE_API_KEY` としてバンドルへ焼き込む（GitHub Variable は不要）。

4. **確認**: デプロイ環境で「資料の追加方法」→「Google ドライブから選ぶ」→ Google ドキュメントを
   1 件取り込み、資料一覧に反映される（API 側で `asset_kind="doc"` の素材として索引される）こと。
   Secret 未投入・未権限の環境では導線が「利用不可」に退化するだけで、ローカルアップロードは無影響。

> リファラ制限は `var.domain` / `var.web_subdomain` から web オリジンを導出する（未設定なら
> Cloud Run の `*.run.app` を許可）。独自ドメイン導入後（§6.5）は apply で許可オリジンが更新される。
...
- [ ] （§6.8）本番は `REQUIRE_ELASTICSEARCH=true` を agent/api/worker に設定済み（ES 未設定・不通なら起動時に fail-fast する / ADR-0064）。
