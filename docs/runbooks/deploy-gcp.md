...
| `NEXT_PUBLIC_GOOGLE_API_KEY` | Google Picker 用のブラウザ API キー（任意 / ADR-0044）。同プロジェクトで Drive API・Picker API を有効化し、HTTP リファラ制限を推奨。未設定なら「Google ドライブから選ぶ」が利用不可の案内になる |
