...
（コメント禁止規約の対象を YAML / Terraform に拡張した経緯は ADR-0067）。
...
- `no-comments` ジョブは説明的コメントの混入を検出する（CLAUDE.md 規約 / ADR-0067）。
...
| `AGENT_MIN_INSTANCES` | agent ワーカーの常駐数。0 の間は音声セッションを開始できないが常時課金も無い。切り替えは agent-standby.yml が gcloud 即時反映とこの Variable の同期更新をまとめて行う。手で変える場合も Variable を更新する（次の apply が巻き戻すため。[pre-launch-cost-controls.md §2](../how-to/pre-launch-cost-controls.md)） |
...
## agent-standby.yml — agent の warm/sleep

プレスリリース前のコスト最小化運用。目的・手順・「Variable 更新 → gcloud 即時反映」の
2 段構えと fail fast の理由は [pre-launch-cost-controls.md §2](../how-to/pre-launch-cost-controls.md) が正。
`concurrency` は terraform apply（`terraform-<ref>`）とは別系統だが、standby の連打は直列化する。
`actions: write` は `AGENT_MIN_INSTANCES` Variable の更新（`gh variable set`）に必要。
