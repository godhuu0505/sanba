...
- 書き込み系のリアルタイム（web → agent）は §4.5 の `user.inquiry_drop` / `user.text`（#185）/ `user.answered`（#181）/ `user.interrupt`（#523）を対象とする。要件確定の永続化は §4 の POST `/finalize`（#186・非リアルタイム）で行い、確認事項の手動 resolve など全般的な双方向操作は**スコープ外**（resolve は会話駆動 / ADR-0059）。
