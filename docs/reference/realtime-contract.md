...
| `user.interrupt` | PTT（押して話す）押下開始（ADR-0066 S3） | `session_id` のみ（付加フィールド無し）。agent は読み上げ中の応答を即時中断する（`session.interrupt()`）。クライアント側 mic ゲート（押下中だけ publish）と対で、エージェント発話へのバージインを決定論的にする。手動ターン（ADR-0073）では `user.turn_start` が置き換える |
| `user.mic_mode` | マイク操作モードの宣言（接続・再接続・切替時 / ADR-0073） | `mode`（`"ptt"` \| `"handsfree"`）, `session_id`。agent は現在の VAD 構成と食い違うときだけセッションを再構築する（PTT=手動 VAD / handsfree=自動 VAD）。既定 PTT と一致するので通常は冪等 |
| `user.turn_start` | PTT 押下＝発話ターン開始（ADR-0073） | `session_id` のみ。agent は読み上げを中断（バージイン）し、手動ターン構成では同時に `activity_start` を送って発話ターンを開く。押している間はサーバ VAD が動かず、無音が続いても途切れない |
| `user.turn_commit` | PTT 離す＝発話ターン確定（ADR-0073） | `session_id` のみ。agent は手動ターンを確定し（`activity_end` + 応答生成）、離した瞬間にエージェントが話し始める |
...
- 書き込み系のリアルタイム（web → agent）は §4.5 の `user.inquiry_drop` / `user.text`（#185）/ `user.interrupt`（#523）/ `user.mic_mode` / `user.turn_start` / `user.turn_commit`（ADR-0073）を対象とする。要件確定の永続化は §4 の POST `/finalize`（#186・非リアルタイム）で行い、確認事項の手動 resolve など全般的な双方向操作は**スコープ外**（resolve は会話駆動 / ADR-0059）。
