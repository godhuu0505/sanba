...
3. **色は意味の写像**。緋＝矛盾／黄土＝抜け などの色は web 側デザイントークンへの**マッピング**であり、
   データ契約のペイロードには載せない（[realtime-contract.md §3](realtime-contract.md) の注記）。色のみに依存せずラベル＋アイコンを併記する。
...
正本（リアルタイム契約）: [`docs/reference/realtime-contract.md`](realtime-contract.md)。
...
| セッション準備 | session prepare | ゴール・役割・参考資料・同意を整える入口画面（Figma 02–04）。 | [screens/02-prepare.md](screens/02-prepare.md) |
...
| 参加トークン | join token / `session_token` | join 済みを証明するトークン。ハイドレーション/起票 API の Bearer に使う（Google idToken とは別物）。 | [realtime-contract.md §4](realtime-contract.md) / [api.ts](../../apps/web/lib/api.ts) |
...
| 発話 ID | `utterance_id` | 発話の識別子（例 `"u3"`）。要件の `citations` と検知の `refs` が**同じ ID 空間**で参照する。リアルタイム契約上の一時 ID であり `Utterance` モデルには永続化しない。 | [realtime-contract.md §3](realtime-contract.md) |
...
> **注**: ADK の sub-agent 名（`contradiction_detector` / `scope_specialist` / `nfr_specialist`）は内部の役割分担。
> 確認事項の realtime 契約（`inquiry.node`）では `detector` フィールドは送らず、種別は `kind` が担う（ADR-0059）。
...
## 5. 確認事項ロジックツリー（矛盾・抜け・確認観点）

「核」のドメイン。会話・解析で見つけた矛盾・抜け・確認観点を**確認事項ノード**として木に載せ、増減・解消する
（ADR-0059）。旧来の平坦な `detection.*` は撤去し `inquiry.node` へ一本化した。

| 用語 | 英語 / 識別子 | 定義 | 出典 |
|---|---|---|---|
| 確認事項ノード | inquiry node / `InquiryNode` | 会話・解析から見つけた問題提起の1ノード。web 内部で `id` 冪等・`seq` 順に正規化する。深さ5・枝5・`confidence` 剪定。 | [inquiry.py](../../packages/sanba_shared/src/sanba_shared/inquiry.py) / [types.ts](../../apps/web/lib/realtime/types.ts) |
| 種別 | `kind` | `contradiction`（矛盾・緋）/ `gap`（抜け・黄土）/ `check`（確認観点・萌黄）/ `ambiguous`（曖昧・藍鼠）。 | [models.py](../../packages/sanba_shared/src/sanba_shared/models.py) |
| 矛盾 | contradiction | 過去の発言・確定要件と食い違う回答（`kind=contradiction`）。色＝**緋**。 | [events.py](../../apps/agent/src/sanba_agent/events.py) |
| 抜け | gap | まだ聞けていない必須論点（`kind=gap`）。色＝**黄土**。 | [events.py](../../apps/agent/src/sanba_agent/events.py) |
| 確認観点 | check | このセッションで必ず確認する観点（`kind=check`・ルート）。coverage 判定でモード別にシード（ADR-0057/0043）。 | [inquiry_feeder.py](../../apps/agent/src/sanba_agent/inquiry_feeder.py) |
| 曖昧 | ambiguous | 具体基準が不明瞭な論点（`kind=ambiguous`）。**advisory**（表示のみ・終了ゲートに算入しない）。 | [events.py](../../apps/agent/src/sanba_agent/events.py) |
| 根拠参照 | `refs` | ノードの根拠となった `utterance_id` の配列。発話までたどれる導線を UI に置く。 | [realtime-contract.md §3](realtime-contract.md) |
| 解消 | resolution | ノードが片付いた状態遷移（`status=resolved`・`inquiry.node` op=resolve）。会話駆動（手動 resolve は無し）。 | [events.py](../../apps/agent/src/sanba_agent/events.py) |
| 剪定 | drop | 誤検知を人手で「不要」にする（`status=dropped`・`user.inquiry_drop`→op=drop）。深さ/枝超過時はサーバが `confidence` 最小を drop。 | [inquiry.py](../../packages/sanba_shared/src/sanba_shared/inquiry.py) |
| 確認事項ツリー | inquiry tree UI | 要件一覧タブのツリービュー（インデント木・kind 別意匠・タップで refs・不要ボタン）。 | [screens/04-conversation.md](screens/04-conversation.md) / [InquiryTree.tsx](../../apps/web/components/InquiryTree.tsx) |
...
| 要件ボード／要件絵巻 | requirement board / scroll | MoSCoW で構造化した成果物ビュー。Figma 演出名「要件絵巻」＝機能名「要件ボード」。 | [screens/06-requirements-scroll.md](screens/06-requirements-scroll.md) / [RequirementsScrollList.tsx](../../apps/web/components/RequirementsScrollList.tsx) |
...
| 用語 | 英語 / 識別子 | 定義 | 出典 |
|---|---|---|---|
| 素材を渡す | provide material | 画像・動画・画面共有・カメラ撮影で情報を渡す入口（05-2 手段選択シート）。 | Figma `148:95` / [screens/05-materials.md](screens/05-materials.md) |
...
| 完成イメージ生成 | generative preview | 確定要件から画面イメージを生成して問い返す出力（拡張・未採用。採用時は ADR を起こす）。 | [design/README.md §3.6](../explanation/design-overview.md) |
...
| Issue 化 | export / `export_requirements_to_github` | 確定要件を GitHub Issue に書き戻す（`POST /api/sessions/{id}/export`）。Figma 演出「奉る」＝機能名「Issue 化」。 | [realtime-contract.md §4](realtime-contract.md) / [api.ts](../../apps/web/lib/api.ts) |
...
| 用語 | 英語 / 識別子 | 定義 | 出典 |
|---|---|---|---|
| データチャネル | data channel | LiveKit ルームの音声と同一接続で JSON イベントを publish する経路。 | [realtime-contract.md §1](realtime-contract.md) |
...
| エンベロープ | envelope | 全イベント共通の枠 `v` / `type` / `seq` / `ts` / `session_id`。 | [realtime-contract.md §2](realtime-contract.md) |
| シーケンス | `seq` | セッション内の単調増加連番。整列・重複排除・欠番検知の基準。 | [events.py](../../apps/agent/src/sanba_agent/events.py) |
| 冪等適用 | idempotent upsert | 同じ `(type, id)` は upsert、`seq` で順序を担保する web 側の適用規則。 | [realtime-contract.md §2](realtime-contract.md) |
| ハイドレーション | hydration | リロード・途中参加時に GET でスナップショットを取得し、データチャネルのライブ差分と合流させる手順。 | [realtime-contract.md §4](realtime-contract.md) |
| 確認事項の剪定 | `user.inquiry_drop` | web→agent の書き込み系。確認事項ノードの「不要」を `node_id` で返す（誤検知の剪定）。 | [realtime-contract.md §4.5](realtime-contract.md) |
...
| 緋 — 矛盾を検知 | 矛盾を検知（`inquiry.node` kind=contradiction） | Figma `40:255` |
...
## 14. UI 用語統一表（UI コピーの正・単一文言 / [ADR-0054](../adr/0054-ui-copy-terminology-unification-and-help.md)）

画面 UI コピーは **モードで分岐せず単一文言**（平易側）に統一する。専門的な正確さはラベルでなくヘルプ（`HelpIcon`）で補う。
end_user モード（[ADR-0032](../adr/0032-guest-join-and-enduser-mode.md)）は **エージェント実行時挙動**（語彙・grounding 出力制御）としてのみ残り、UI 文言は分岐しない。
画面別の詳細対応は [`supplements/0054-copy-and-help-map.md`](../adr/supplements/0054-copy-and-help-map.md)。

| 概念 | 正語（単一・UI） | 排除・置換する語 |
|---|---|---|
| 会話を始める行為 | 会話を始める | 壁打ちを始める / 要件サンバを始める / 深掘りを開始する |
| 会話（画面・行為・単位） | 会話 | 問答 / セッション（UI）/ ルーム |
| 1回の参加リンク | 会話リンク | 深掘りリンク / 参加リンク |
| 役割 | 利用者 / 企画者 / 開発者 | 顧客 / 企画(PdM) / エンジニア |
| 検知 | 食い違い / 確認したい点 / あいまい | 矛盾 / 抜け / 不明瞭 |
| 未解決の件数・一覧 | 未解消 / 確認したいこと | 未確定 / 深掘り対象 |
| 確定/暫定 | 確定した要件 / 未確定の要件 | 確定要件・暫定要件 / 確認できた内容・途中までの内容 |
| 要件の一覧 | 要件一覧 | 要件絵巻 / 過去の要件 |
| 優先度 | ぜひ必要 / あると助かる / できれば / 今回は見送り | Must必須 / Should望ましい …（MoSCoW はヘルプ） |
| 分類 | 機能 / 使い心地 / 前提 / 範囲 / 確認中 | 非機能 / 制約 / 境界 / 未解決 |
| 確からしさ | 確信度（+ヘルプ） | 確からしさ |
| 発言の主 | 発言者 | 出所 / だれの発言か |
| 参考資料 | 参考資料（アップ種別の"資料"は文書） | 素材 / 資料 |
| 言葉と画の不一致 | 言葉と画像の食い違い | 言葉×画の矛盾 |
| 認証 | ログイン / 新規登録 | サインイン / サインアップ |
| 秘匿処理 | 伏せ字にします | マスクします |
| 音声出力 | スピーカー消音 | 消音（単独） |

共有ラベル定数は `apps/web/lib/realtime/mapping.ts`（検知/分類/優先度）・`apps/web/lib/audience.ts`（役割）に集約。
ヘルプ本文は `apps/web/lib/help/`。プロダクト名「SANBA」は残す（排除するのは行為の愛称「要件サンバ」）。
§13 の演出語対応表は Figma 由来の履歴。UI コピーの現行の正は本節（§14）とする。
