...
| 発話 ID | `utterance_id` | 発話の識別子（例 `"u3"`）。要件の `citations` と検知の `refs` が**同じ ID 空間**で参照する。 | [realtime-contract.md §3](design/realtime-contract.md) / [models.py](../packages/sanba_shared/src/sanba_shared/models.py) |
...
| カテゴリ | `category` / `RequirementCategory` | 要件の種別。正準値は [§8](#8-列挙の正準値) 参照。 | [models.py](../packages/sanba_shared/src/sanba_shared/models.py) |
