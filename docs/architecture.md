...
| Phase | 参加者 | エージェント | 識別 |
|---|---|---|---|
| 1 (MVP) | 1 | 1 (Lead のみ) | 不要 |
| 2 | N | 1 | 話者識別（LiveKit participant identity + 声紋/トラック単位） |
| 3 | N | M (専門 sub-agent が並行) | 役割割当・発話の宛先制御 |

多人数では「誰の発話か」を participant identity に紐付けて Firestore に保存し、要件の出所をトレースできるようにする。多エージェントでは、各専門エージェントが**特定の論点が出たときだけ**割り込む（割り込みポリシーは ADR で定義予定）。
