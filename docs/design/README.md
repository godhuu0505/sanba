...
> 色は意味と一対一で対応（紫→金、ティール→橄欖、琥珀→黄土、赤→緋）させたまま移し替えるため、Must/Should/Could・矛盾/抜け・エージェント種別などの**識別性は保持**される。実装時は §4 のトークンを正とする。
>
