...
**全モックアップにアートディレクション「産婆術アトリエ」（金箔／油彩／最後の晩餐の背景／明朝）を適用済み**（§0 参照。情報設計・画面構成・セマンティクスは不変、意匠のみ差し替えた"スキン"）：

- 🖥 **デスクトップ版**：➡️ **[`mockups/sanba-ux.html`](mockups/sanba-ux.html)**（6画面＋デザインシステム）
...
open docs/design/mockups/sanba-ux.html
open docs/design/mockups/sanba-ux-responsive.html
open docs/design/mockups/sanba-ux-multimodal.html
open docs/design/mockups/sanba-ux-atelier.html
```
...
| 種別 | 内容 | リンク |
|---|---|---|
| 🎨 Design（**正本**） | 「📱 iPhone 13 Pro 操作フロー・正本」モバイル縦・全10画面の一本道（01ホーム→…→10完了） | [figma.com/design/eI6QvvCEO021zpdMmxr8Iq?node-id=31-2](https://www.figma.com/design/eI6QvvCEO021zpdMmxr8Iq?node-id=31-2) |
...
> ⚠️ HTML モック（`mockups/*.html`）は**デスクトップ3分割の参考資料**であり、正本ではない。正本はモバイル10画面（上表＋[`screens/`](screens/)）。

> Figma → コード（`apps/web` 実装）、コード → Figma（同期）の双方向ループは ADR-0011 の通り MCP で回す。

---

## 0. アートディレクション「産婆術アトリエ（Gilded Maieutics）」
...
> 意匠の典拠は Web 調査に基づく（下記ソース）。テーマ原典は `sanba-ux-atelier.html`。
> **既存の機能設計（IA・画面・コンポーネント・状態セマンティクス）は一切変えず、見た目だけを差し替える「スキン」として全モックアップ（desktop / responsive / multimodal）へ適用済み**。
> 適用は機械的な置換で行った（紫→金、ティール→橄欖、琥珀→黄土、赤→緋、白罫線→金罫線、サンセリフ→明朝、背景に最後の晩餐の内陣＋ニスのひび）。色は意味と一対一で対応させたまま移し替えているため、Must/Should/Could・矛盾/抜け・エージェント種別などの**識別性は保持**される。
>
...
正本は **モバイル縦・一本道の10画面**。「準備 → 対話 → 検知 → 素材 → 解析 → 確定 → 完了」を分断しない直線フロー。
各画面の要件は [`screens/`](screens/) を参照。
...
| 画面 | 役割 | 優先度 | 要件票 |
|---|---|---|---|
| 01 ホーム | 入口・価値訴求・実績 | P2 | [01](screens/01-home.md) |
| 02 準備 | ゴール/役割/資料(RAG)/同意 | P1 | [02](screens/02-prepare.md) |
| 03 待機 / 04 発話中 | 音声ファーストの対話 | P1 | [03](screens/03-live-idle.md) / [04](screens/04-live-speaking.md) |
| 05 検知 | **核**：矛盾/抜けをその場で取り上げ | **P0** | [05](screens/05-detection.md) |
| 06 素材 / 07 アップロード中 | マルチモーダル入力（ADR-0004） | **P0** / P1 | [06](screens/06-material.md) / [07](screens/07-uploading.md) |
| 08 解析結果 | 抽出要件＋言葉×画の矛盾 | **P0** | [08](screens/08-analysis.md) |
| 09 要件絵巻 | MoSCoW 成果物＋Issue化 | **P0** | [09](screens/09-scroll.md) |
| 10 完了 | 締め（オーレ！） | P1 | [10](screens/10-complete.md) |
...
ビジュアルは [`mockups/sanba-ux-responsive.html`](mockups/sanba-ux-responsive.html)（スマホ5画面＋タブレット）参照。
...
ビジュアルは [`mockups/sanba-ux-multimodal.html`](mockups/sanba-ux-multimodal.html) 参照。
