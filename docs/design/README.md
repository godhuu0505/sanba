...
- 🏛✨ **テーマ版「産婆術アトリエ」**：➡️ **[`mockups/sanba-ux-atelier.html`](mockups/sanba-ux-atelier.html)**（金箔／油彩／明朝のアートディレクション。下記 §0 参照）
...
> 意匠の典拠は Web 調査に基づく（下記ソース）。本テーマは `sanba-ux-atelier.html` に実装。既存の機能設計（IA・画面・コンポーネント）はそのまま、見た目だけを差し替える「スキン」として全画面へ展開できる。
>
