...
> 状態: **Reviewed（設計確定・実装待ち）**。初版 Draft を 3 観点（ADR 整合・セキュリティ / 会話設計・UX /
> 実装実現性）で相互レビューし、owner ヒアリング（2026-07-06）で主要判断を確定した改訂版。
> 実装開始時に ADR を起票する論点は [§8](#8-確定事項と-adr-起票) に整理する。
...
- **B（不具合）の出口 — 確定（ヒアリング）**: 自動 Issue 起票はしない。再現手順つき inquiry として
  ...
  矛盾しない。export 時は `requirements_to_issue_body` を拡張し、classification=bug の inquiry を
  「再現手順つきバグ報告」セクション＋ラベルとして整形する。
...
| PR | 内容 |
|---|---|
| A-0 | ADR 起票（§8）: inquiries 新設・kind=support と allowlist 改訂・構造化単発呼び出し・誤案内ガード |
...
| A-4 | `GET /api/sessions/{id}/inquiries` ＋ export の bug セクション整形 |
...
本書のレビューとヒアリング（2026-07-06）で確定した判断。実装開始時（Stage A の PR A-0）に
ADR として起票する（番号は起票時に採番）。
