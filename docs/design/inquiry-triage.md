...
> **[ADR-0040](../adr/0040-inquiry-triage-and-support-knowledge.md)**（本書と食い違う場合は ADR が正）。
> 本書は課題定義・会話設計・実装計画の詳細を持つ。初版 Draft を 3 観点（ADR 整合・セキュリティ /
> 会話設計・UX / 実装実現性）で相互レビューし、owner ヒアリング（2026-07-06）で主要判断を確定、
> ADR-0040 として起票した（[§8](#8-確定事項と-adr) に一覧）。
...
設計判断の ADR（[ADR-0040](../adr/0040-inquiry-triage-and-support-knowledge.md)）は本設計文書と
同時に起票済み。実装 PR は ADR-0040 を参照して進める。
...
**[ADR-0040](../adr/0040-inquiry-triage-and-support-knowledge.md) として起票済み**の判断
