...
> 画面ごとの実装可能な要件票は **[`screens/`](../design/screens)**、リアルタイム伝送と状態復元の契約は
...
> | 成果物 | 画面別要件票（目的/UI要素/状態/連携/受け入れ基準）= [`screens/`](../design/screens) |
...
[`screens/`](../design/screens) を参照する。意匠（色・書体・様式）は ADR-0033 と `apps/web/app/globals.css` が正。
...
各画面の要件は [`screens/`](../design/screens)、会話フェーズの詳細は [`conversation-experience.md`](../reference/conversation-experience.md) を参照。
...
2. **apps/web 実装** — [`screens/`](../design/screens) の要件票を React 化し LiveKit Components / データチャネルと接続。
