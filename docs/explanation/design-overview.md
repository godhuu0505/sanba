...
> **⚠ 正本の逆転・アートディレクション刷新（[ADR-0047](../adr/0047-app-is-canonical-figma-follows.md) /
...
> - **正本＝`apps/web` の実装**（Figma は追従する follower。ADR-0047）。
...
> 実装・Figma の意匠を直す際の正には使わない。Figma 再構成の指針は [`figma-redesign-spec.md`](../notes/figma-redesign-spec.md)。
...
>
> **Superseded（ADR-0047）**: 下記の「Figma が唯一の正本」「Figma モバイル縦10画面が正本」という宣言は**過去のもの**。
> 現在の正本は **`apps/web` の実装**（ADR-0047。Figma は追従する follower）。
...
> 画面ごとの実装可能な要件票は **[`screens/`](../reference/screens)**、リアルタイム伝送と状態復元の契約は
> **[`realtime-contract.md`](../reference/realtime-contract.md)** に分離した。
> 会話フェーズ（03〜08）の刷新案は **[`conversation-experience.md`](../reference/conversation-experience.md)**
...
各画面の要件は [`screens/`](../reference/screens)、会話フェーズの詳細は [`conversation-experience.md`](../reference/conversation-experience.md) を参照。
...
| 系 | 役割 | 優先度 | 要件票 |
|---|---|---|---|
| 01 ホーム | 入口・価値訴求 | P2 | [01](../reference/screens/01-home.md) |
| 02 準備 | ゴール/役割/資料(RAG)/同意 | P1 | [02](../reference/screens/02-prepare.md) |
| 03 会話開始 | 開始前/接続/録音許可/失敗 | P1 | [03](../reference/screens/03-conversation-start.md) |
| 04 会話履歴 | **核**：音声対話・選択肢3モード・矛盾/抜け検知 | **P0** | [04](../reference/screens/04-conversation.md) |
| 05 参考資料 | **核**：投入・背景解析・言葉×画の矛盾 | **P0** | [05](../reference/screens/05-materials.md) |
| 06 要件絵巻 | **核**：MoSCoW・確信度/出所（閲覧のみ） | **P0** | [06](../reference/screens/06-requirements-scroll.md) |
| 07 判定 | 確定ゲート（未解消0で確定） | P1 | [07](../reference/screens/07-judgment.md) |
| 08 結果 | 産婆結果＋出力（PDF/Drive/Issue） | P1 | [08](../reference/screens/08-result.md) |
...
生成は**非同期で会話を止めない**（二層構造を踏襲, ADR-0002）。所要時間/コストは Cloud Trace / Cloud Monitoring で観測する（ADR-0051）。
...
1. **P0 から着手** — 08 解析結果 / 06 素材 / 05 検知 / 09 要件絵巻。先に
   [`realtime-contract.md`](../reference/realtime-contract.md) の追加 API（`GET /requirements`、`POST /export`）と
   データチャネル publish を入れる。
2. **apps/web 実装** — [`screens/`](../reference/screens) の要件票を React 化し LiveKit Components / データチャネルと接続。
3. **計測の配線** — 検知数・確信度・要件数を OTel（Cloud Trace）と構造化ログ（Cloud Monitoring）に通し評価（ADR-0005 / ADR-0051）へ供給。
