...
| 観測性 | `inquiry_classified`（session / classification / confidence / trigger）/ `support_guided`（entry_id）/ `guide_verified`（success/fail）を構造化ログ＋トレースへ。LLM 入出力は構造化ログ + Cloud Trace（CLAUDE.md 原則3, ADR-0051）。heuristic 無効（非 ja）も構造化ログで可視化 | — |
...
評価は CI 回帰評価データセット（Gemini judge）で回す方針で、外部データセット基盤には依存しない（ADR-0051）。トリアージ評価もまず同じ形に載せる:
...
- **教師信号の回収**: owner が集約ビューで分類・outcome を修正したら
  ...
  CI 回帰評価データセットに載せるのは **mask_pii 済みかつ owner が確認したケースのみ**。
...
CI 回帰評価データセットの拡充時期（ADR-0005 の TO-BE と同期）。
