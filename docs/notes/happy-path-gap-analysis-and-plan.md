...
| # | 論点 | 推奨 |
|---|------|------|
| Q1 | 着手順: Phase 0（安定化）から入るか、Phase 1（体験）と並行するか | Phase 0 → 1 → 2 → 3 の直列。ただし P1-1(a) と P1-3 は独立なので並行可 |
| Q2 | HP9 の結果生成: 現行の機械整形（ADR-0042、決定済・注入耐性あり）を維持するか、「会話ログ全体を AI に食わせて生成」へ変更するか | 機械整形を維持し、要約系プレースホルダ（`{{conversation_summary}}` 等）を LLM で追加生成するハイブリッド |
| Q3 | ロジックツリー（Phase 2）を今回スコープに含めるか、フラット検知＋確認項目シードの現行近似で先に全体を通すか | まず Phase 0/1 で一気通貫を確保し、その後 Phase 2 を ADR から着手 |
| Q4 | Issue への会話ログ同梱の既定（on/off）と粒度（全文/要約） | 既定 off・要約のみ。全文はオプション |
